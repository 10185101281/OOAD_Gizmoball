...
public class XCircle extends XComponent {
    ...
    public void judge(BouncingBall ball) {
        ...
        if(distance<(length/2+ball.getRadius()))//如果圆心距离小于半径和，则相交
        {
            ...
            System.out.print("("+center_x1+","+center_y1+")");
            System.out.print("("+center_x2+","+center_y2+")");
            System.out.println("距离："+distance);
            System.out.println(length/2+ball.getRadius());
            System.out.println("old:("+ball.getX()+","+ball.getY()+")");
            System.out.println("before:x:"+ball.getVx()+" y:"+ball.getVy());
            ...
            System.out.println("new:("+ball.getX()+","+ball.getY()+")");
            react(ball);
            System.out.println("after:x:"+ball.getVx()+" y:"+ball.getVy());
        }
    }
    ...
    public void react(BouncingBall ball) {
        ...
        //切向速度和法向速度再沿xy轴分解
        double vytx;//切向速度在x轴投影
        double vyty;//切向速度在y轴投影
        double vynx;//法向速度在x轴投影
        double vyny;//法向速度在y轴投影
        if((center_x2>center_x1&&center_y2<center_y1)||(center_x2<center_x1&&center_y2>center_y1))//第一、四象限
        {
            vytx = vyt * cos_a;
            vyty = vyt * sin_a;
            vynx = -vyn*sin_a;
